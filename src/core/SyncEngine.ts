...
export class SyncEngine {
  ...
  async run(options: SyncOptions): Promise<void> {
    ...
    const candidates = await this.source.listCandidateMessages(checkpoint, { folders: options.sourceFolders });
    ...
    for (const msg of candidates) {
      ...
      try {
        ...
        await pRetry(async () => {
          this.logger.info(`Processing message [${processedCount}/${candidates.length}]: ${msg.id} - ${msg.subject}`);
          ...
          const hashSeen = await this.state.hasSeen(this.source.name, accountId, msg.id, contentHash);
          if (hashSeen) {
          ...
          await this.destination.storeRawMessage(rawMime, msg, { targetMailbox: options.targetMailbox });
          ...
          if (!latestCheckpoint.lastReceivedAt || msg.receivedAt.toISOString() > latestCheckpoint.lastReceivedAt) {
          ...
          retries: 3,
          onFailedAttempt: err => {
            this.logger.warn(`Attempt ${err.attemptNumber} failed for message ${msg.id}: ${err.error.message}`);
          }
        });
      } catch (err: any) {
    ...
    this.logger.info(`Sync completed: ${successCount} success, ${skipCount} skipped, ${errorCount} errors`);
